...
is a vector field pointing toward the highest density regions, where data is more likely to exist at a given noise level.
...
Instead of learning the clean distribution, we learn the noisy image distribution:
...
That is a noise component that is more aligned for a specific class.
...
where $\gamma > 1$ to amplify the signal. This shifts probability mass from the least likely to the most likely class values, meaning the greater the $\gamma$, the more class consistent the generation. $\gamma$ is the inverse temperature parameter.
...
Instead of training a separate classifier, we train a single diffusion model that can operate both conditionally and unconditionally. This is done using conditioning dropout, where the conditioning variable $y$ is randomly removed during training. When $y$ is removed, the model learns to behave like an unconditional model.
...
\hat{\epsilon}(x_t, t, y)
=
(1 - \gamma)\,\epsilon_\theta(x_t, t, \phi)
+
\gamma\,\epsilon_\theta(x_t, t, y)
$$
